...
public:
    ...
    void showMeCar() {
        cout << "Марка: " << getMark(mark) << endl;
        cout << "Название: " << name << endl;
        cout << "Класс: " << getTheClass(theClass) << endl;
        cout << "Руль: " << getPosition(position) << endl;
        cout << "Привод: " << getDrive(drive) << endl;
        cout << "ЛС: " << hp << endl;
        cout << "Цвет: " << getColor(color) << endl;
    }
};
...
int main () {
    ...
    carBuilder.setName("Gocha's Silvia S-15");
    carBuilder.setTheClass(TheClass::sport);
    carBuilder.setPosition(Position::rightHanded);
    carBuilder.setDrive(Drive::rwd);
    carBuilder.setHp(490);
    carBuilder.setColor(Color::yellow);
